...
int path=0;
if(argc==1){
	printf("Reads Qualcomm mif program metadata files. (.bar)\nType \"dumpmif --help\" for help.\n");
	exit(0);
}
for(int i=1;i<argc;i++){
	if(!strcmp(argv[i],"--help")){
		printf("Usage: dumpmif [options] -o <file>\n");
		exit(0);
	}
	if(!strcmp(argv[i],"-o")){
		if(i+1>argc){
...
for(int i=0;i<3;i++){
	if(header[i]!=buffer[i]){
		printf("May or may not be a .mif file\n"); //check file header.
		exit(0);
	}
}
...
for(int i=0;i<total_count;i++){
	printf("Mif resource %i type: 0x%04x\n",i+1,toc[i].type);
	printf("Mif resource %i id: 0x%04x\n",i+1,toc[i].id);
	printf("Mif resource %i length?: 0x%08x\n",i+1,toc[i].length);
}
rewind(fp);
fseek(fp,(long)16,SEEK_SET);
uint32_t offset_table_offset[]={0,0,0};
...
printf("Number of mif resources %i\n",offset_table_len);
...
for(int i=0;i<offset_table_len;i++){
	...
	sprintf(output_name,"mifrsc0x%04x.%i",toc[i].id,toc[i].type);
	if(i<offset_table_len-1){
		sprintf(dd,"dd if=%s of=%s bs=1 skip=%i count=%i status=progress",argv[path],output_name,offset_table[i],offset_table[i+1]-(offset_table[i]));
	}else{
		sprintf(dd,"dd if=%s of=%s bs=1 skip=%i status=progress",argv[path],output_name,offset_table[i]);
	}
	printf("%s\n",dd);
	system(dd);
}
